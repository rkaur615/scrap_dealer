                            ...
                            @foreach ($myQuotes as $quote)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a id="rating_btn" onclick="updateRequirementId({{$quote->requirement_id}},{{$quote->requirement->user_id}})" class="btn btn-warning btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="Feedback"><i class="fa fa-commenting-o"></i></a>&nbsp;
                                        @endif
                                        <a href="{{route('chat.rooms',[$quote->requirement->user_id,$quote->requirement_id])}}" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="Chat"><i class="fa fa-comments-o"></i></a>&nbsp;
...
let updateRequirementId = async function(id, sid){
    console.log("req.", id)
    $("#requirement_id").val(id);
    $("#supplier_id").val(sid);
    let response = await fetch("/user/details/"+sid+"/"+id)
    let res = {};
    try {
    ...
    console.log("here is fetch response", res)

    $("#upic").attr("src",`/storage/${res.data.pic}`)
...
$("#submitRating").on('click', function(){
    ...
    (async () => {
        const rawResponse = await fetch('/user/requirement/saveRating', {
            method: 'POST',
            headers: {
